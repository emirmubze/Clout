...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if is_postgres and sqlite_source.exists():
            ...
            if pg_user_count == 0 and pg_course_count == 0:
                self.stdout.write(self.style.NOTICE("==> PostgreSQL database is empty. Migrating seed data from SQLite..."))
    ...
    def _import_from_sqlite(self, sqlite_path):
        try:
            ...
            for row in modules:
                ...
                course = course_map.get(course_id) or Course.objects.first()
                if not course:
                    continue
